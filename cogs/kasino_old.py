...
class Kasino(commands.Cog):
    ...
    @commands.cooldown(1, 60, type=commands.BucketType.default)
    async def openkasino(self, ctx, *args):
        ...
        to_embed = discord.Embed(description="Opening kasino, hold tight...")
    ...
    async def refreshkasino(self, ctx):
        ...
        to_embed = discord.Embed(description="Refreshing kasino, hold tight...")
        kasinomsg = await ctx.send(embed=to_embed)
    ...
    async def closekasino(self, ctx, arg):
        ...
        if int(arg) == 3:
            for b in a_bets:
                ...
                self.bot.get_cog('Karma').change_state_user(b[0], 'upvote', 'post', postAmount, cursor)
                self.bot.get_cog('Karma').change_state_user(b[0], 'upvote', 'link', b[1]-postAmount, cursor)
        ...
        # OPTION 1 WINS
            ...
            for b in a_bets:
                ...
                self.bot.get_cog('Karma').change_state_user(b[0], 'upvote', 'post', postAmount, cursor)
                self.bot.get_cog('Karma').change_state_user(b[0], 'upvote', 'link', totalWon - postAmount, cursor)
                user = cursor.execute(f'SELECT post_karma, link_karma FROM users WHERE user_id={b[0]}').fetchone()
                output = discord.Embed(
